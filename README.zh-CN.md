...
- **smallModel：** 无工具预热消息的回退模型（例如 Claude Code 的探测请求）；默认是 `gpt-5-mini`。网关会对无工具的预热或探测请求强制使用该小模型，以避免消耗 premium 请求。该行为仅在 GitHub Copilot 账户为非 token-based 计费时生效（`token_based_billing` 为 false）；对于 token-based 计费账户，预热小模型回退会被跳过，因为不存在需要节省的 premium 请求配额。
  > **`AUTO_MODE_MODEL`（环境变量，不是 `config.json` 中的字段）：** 覆盖处理上述无工具预热/探测流量所使用的模型。默认未设置，此时行为与上面 `smallModel` 描述的行为完全一致。设置后，无论 `token_based_billing` 为何值，都会无条件将该流量重定向到指定模型，其优先级高于 `smallModel`。这类无工具请求的结构也被 Claude Code 的 Auto Mode 后台分类器所使用（经过实测验证，没有可靠的方法能将分类器请求与普通预热探测请求区分开），因此该设置也决定了由哪个模型来评估 Auto Mode 的风险操作审批。**注意：** 如果重定向到能力不足的模型，可能导致 Auto Mode 的分类器判定失败（fail closed）从而阻止操作（或报"模型不可用"错误）；请在此选择一个足够强大的模型，不一定要与 `smallModel` 相同。
