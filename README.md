...
- **Multi-account request routing**: Add multiple GitHub Copilot accounts using `auth add`.
  ...
  - **Free models**: By default, account affinity routes requests from the same session to the same account. On the first request for a new session, round-robin distributes across eligible accounts. Set `accountAffinity=false` in `config.json` to disable affinity and route all requests sequentially.
