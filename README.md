...
- **useResponsesApiWebSocket:** When `true`, Copilot Responses requests use WebSocket for models that advertise `ws:/responses`; models that advertise only `/responses` use HTTP. Streamed Responses requests for the built-in `codex` provider use WebSocket whenever this setting is enabled, while non-streaming Codex requests always use HTTP. Set this to `false` to make Copilot use HTTP `/responses` where the selected model advertises it and to send streamed Codex Responses requests over HTTP. WebSocket failures invalidate the failed connection. For requests that support HTTP Responses and do not depend on `previous_response_id`, subsequent requests for the same session, model, and credentials use HTTP/SSE for five minutes before trying WebSocket again. Partially delivered requests are never replayed automatically; send another message in the existing session or let the client retry. User cancellation does not trigger fallback, and WebSocket-only models open a fresh connection on the next request. Defaults to `true`. If a proxy, VPN, or network blocks or destabilizes WebSocket traffic, disable this setting or switch networks.
