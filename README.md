...
- **Claude Desktop Compatibility**: Claude Desktop's developer settings allow configuring alternative API gateways, but its model name input only accepts names that contain the word `claude` or match official Anthropic models. To work around this restriction, append `_claude_desktop` to any Copilot model name when entering it in Claude Desktop (e.g. `gpt-4.1_claude_desktop`). The proxy automatically strips this suffix before forwarding requests upstream, so the correct Copilot model is used transparently.
