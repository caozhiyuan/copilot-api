...
- **smallModel:** Fallback model used for tool-less warmup messages (e.g., Claude Code probe requests); defaults to gpt-5-mini. The gateway forces this small model on no-tool warmup or probe requests to avoid consuming premium requests. This behavior only applies to non-token-based-billing GitHub Copilot accounts (`token_based_billing` is false); for token-based-billing accounts the warmup small-model fallback is skipped since there is no premium-request quota to preserve.
  > **`AUTO_MODE_MODEL` (environment variable, not a `config.json` key):** Overrides which model handles the tool-less warmup/probe traffic described above. Unset by default, in which case behavior is unchanged from the `smallModel` description above. When set, it redirects this traffic to the specified model unconditionally, regardless of `token_based_billing`, taking precedence over `smallModel`. This same tool-less request shape is also used by Claude Code's Auto Mode background classifier (verified empirically that there is no reliable way to distinguish a classifier call from a generic warmup ping), so this setting also determines which model evaluates Auto Mode's risky-action approvals. **Caution:** redirecting to a model that isn't capable enough may cause Auto Mode's classifier to fail closed and block actions (or report the model as unavailable); choose a capable model here — it does not need to match `smallModel`.
