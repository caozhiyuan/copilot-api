...
bunx lint-staged     # Lint & fix staged files (pre-commit hook)
```

## Architecture

This is a reverse-engineered proxy that exposes the GitHub Copilot API as both an OpenAI-compatible and Anthropic-compatible HTTP service. The entry point is `src/main.ts` (CLI via `citty`), which dispatches to subcommands: `start`, `stop`, `auth`, `check-usage`, `debug`.
...
- **Imports:** Use `~/` alias for `src/` (e.g., `import { foo } from '~/lib/foo'`). ESNext syntax, no CommonJS.
- **Formatting:** Prettier (with `prettier-plugin-packagejson`). Run `bun run lint` to auto-fix.
- **TypeScript:** Strict mode — no `any`, `noUnusedLocals`, `noUnusedParameters`. Use explicit types and interfaces.
- **Naming:** `camelCase` for functions/variables, `PascalCase` for types/interfaces.
- **Error handling:** Use explicit error classes — `HTTPError` from `src/lib/error.ts`; route handlers catch and call `forwardError(c, error)`. Avoid silent failures.
...
- **Linting:** Local flat config in `eslint.config.js` — JS recommended, TS recommended type-checked, unused-import cleanup, and Prettier.
- **Testing:** Bun's built-in test runner. Place tests in `tests/`, name as `*.test.ts`.

## Commit & PR Conventions

- Never add "Generated by", "Co-Authored-By", "mai-agents", "claude", or "codex" lines in commits or PR descriptions.
